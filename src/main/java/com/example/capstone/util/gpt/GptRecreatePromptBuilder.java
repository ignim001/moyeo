...
응답 형식:
- 반드시 JSON으로 반환. 다른 문장이나 설명은 절대 추가하지 마. JSON외의 것을 응답시 파싱 불가!
- 설명, 안내 문구, 백틱(```), 마크다운 코드블럭은 절대 포함하지 마. 
...
응답을 작성한 후 반드시 다음을 검토할 것:
- 위에 명시된 제외 장소가 단 1개라도 포함되어 있다면 전체 응답은 실패로 간주됨
- 모든 일정에서 제외 장소가 누락되었는지 GPT 스스로 확인하고, 포함되었으면 다시 생성할 것
